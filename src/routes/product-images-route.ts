...
export const productImagesRoute = new Elysia({
  prefix: '/api',
  tags: ['Product Images'],
})
  .onError(({ error, set }) => {
  ...
  .post(
    ...
    async ({ body, set, headers }) => {
      ...
      try {
        ...
        if (
          error.message === 'Images must not be empty' ||
          error.message === 'Exactly one image must be set as primary'
        ) {
          set.status = 422;
  ...
  .get(
    '/product-images/:variantId/current',
    async ({ params, set, headers }) => {
      const authHeader = headers['authorization'] || headers['Authorization'];
      ...
      try {
        ...
        if (
          ...
          error.message === 'Primary image not found'
        ) {
          set.status = 404;
          return { error: error.message };
        }
