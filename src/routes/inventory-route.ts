...
const createInventoryRoute = new Elysia()
  .post(
    '/api/inventory',
    async ({ body, set, headers }: any) => {
      ...
      try {
        ...
        if (err.message.includes('not found') || err.message.includes('must be less than')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
    ...
    },
    {
      ...
      detail: {
        ...
        responses: {
          ...
          409: {
            description: 'Inventory for this variant and warehouse already exists',
            content: {
              'application/json': {
                example: {
                  error: 'Inventory untuk varian dan gudang ini sudah ada'
                }
              }
            }
          },
...
const listInventoryRoute = new Elysia()
  .get(
    '/api/inventory',
...
const getInventoryDetailRt = new Elysia()
  .get(
    ...
    async ({ params, set, headers }: any) => {
      ...
      try {
        ...
        if (err.message.includes('Inventory tidak ditemukan')) {
          set.status = 404;
          return { error: err.message };
        }
        throw err;
...
const updateInventorySettingsRoute = new Elysia()
  .patch(
    '/api/inventory/:variantId/:warehouseId',
    async ({ params, body, set, headers }: any) => {
      ...
      try {
        const result = await updateInventorySettingsSvc(variantId, warehouseId, body);
        return { data: result };
      } catch (err: any) {
        if (err.message.includes('Inventory tidak ditemukan')) {
          set.status = 404;
        ...
        if (err.message.includes('Minimum stock must be less than maximum stock')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
...
const adjustStockRoute = new Elysia()
  .post(
    ...
    async ({ params, body, set, headers }: any) => {
      ...
      try {
        ...
        if (err.message.includes('Stok tidak mencukupi') || err.message.includes('Stok tidak boleh kurang dari jumlah yang direservasi') || err.message.includes('Adjustment quantity cannot be zero')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
...
const reserveStockRoute = new Elysia()
  .post(
    ...
    async ({ params, body, set, headers }: any) => {
      ...
      try {
        ...
        if (err.message.includes('Stok tidak mencukupi untuk direservasi')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
...
const releaseStockRoute = new Elysia()
  .post(
    ...
    async ({ params, body, set, headers }: any) => {
      ...
      try {
        ...
        if (err.message.includes('Jumlah pelepasan melebihi stok yang direservasi')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
...
const deleteInventoryRoute = new Elysia()
  .delete(
    '/api/inventory/:variantId/:warehouseId',
    async ({ params, set, headers }: any) => {
      const authHeader = headers['authorization'] || headers['Authorization'];
      ...
      try {
        ...
        if (err.message.includes('Tidak dapat menghapus inventory yang masih memiliki reservasi aktif')) {
          set.status = 422;
          return { error: err.message };
        }
        throw err;
        ...
        responses: {
          ...
          422: {
            ...
            content: {
              'application/json': {
                example: {
                  error: 'Tidak dapat menghapus inventory yang masih memiliki reservasi aktif'
                }
              }
            }
          },
